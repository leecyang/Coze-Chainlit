...
const stopStreamingMessages = (messages: IStep[]): IStep[] => {
  ...
  const nextMessages = messages.map((message) => {
    const updatedSteps = message.steps
      ? stopStreamingMessages(message.steps)
      : undefined;
    const shouldStopStreaming = message.streaming;
    const stepsChanged = updatedSteps && updatedSteps !== message.steps;

    if (!shouldStopStreaming && !stepsChanged) {
      return message;
    }
    ...
    return {
      ...message,
      ...(shouldStopStreaming ? { streaming: false } : {}),
