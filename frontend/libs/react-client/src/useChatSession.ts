...
const useChatSession = () => {
  ...
  const streamTokenQueueRef = useRef<IToken[]>([]);
  const streamTokenFrameRef = useRef<number | null>(null);

  const [currentThreadId, setCurrentThreadId] =
    useRecoilState(currentThreadIdState);

  const flushStreamTokens = useCallback(() => {
    if (streamTokenFrameRef.current !== null) {
      cancelAnimationFrame(streamTokenFrameRef.current);
      streamTokenFrameRef.current = null;
    }

    const queuedTokens = streamTokenQueueRef.current;
    if (!queuedTokens.length) return;

    streamTokenQueueRef.current = [];
    const mergedTokens = new Map<string, IToken>();
    const mergedTokenKeys: string[] = [];

    queuedTokens.forEach((streamToken) => {
      const key = `${String(streamToken.id)}:${
        streamToken.isInput ? 'input' : 'output'
      }`;
      const existing = mergedTokens.get(key);

      if (!existing) {
        mergedTokens.set(key, { ...streamToken });
        mergedTokenKeys.push(key);
      } else if (streamToken.isSequence) {
        existing.token = streamToken.token;
    ...
    const tokensToApply = mergedTokenKeys.map((key) => mergedTokens.get(key)!);

    setMessages((oldMessages) =>
      tokensToApply.reduce(
        (messages, { id, token, isSequence, isInput }) =>
          updateMessageContentById(
            messages,
            id,
            token,
  ...
  const scheduleStreamTokenFlush = useCallback(() => {
    if (streamTokenFrameRef.current !== null) return;

    streamTokenFrameRef.current = requestAnimationFrame(() => {
      streamTokenFrameRef.current = null;
      flushStreamTokens();
    });
  }, [flushStreamTokens]);
  ...
  const _connect = useCallback(
    async ({
      ...
      socket.on(
        'stream_token',
        (streamToken: IToken) => {
          streamTokenQueueRef.current.push(streamToken);
