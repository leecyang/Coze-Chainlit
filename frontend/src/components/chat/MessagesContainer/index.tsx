...
import WaitingResponse from '@/components/chat/Messages/WaitingResponse';
...
const MessagesContainer = ({ navigate }: Props) => {
  ...
  const hasUserMessage = useMemo(() => {
    const containsUserMessage = (steps: IStep[]): boolean =>
      steps.some(
        (step) =>
          step.type === 'user_message' ||
          containsUserMessage(step.steps || [])
      );

    return containsUserMessage(messages);
  }, [messages]);

  const hasStreamingAssistantOutput = useMemo(() => {
    const containsStreamingAssistantOutput = (steps: IStep[]): boolean =>
      steps.some(
        (step) =>
    ...
    return containsStreamingAssistantOutput(messages);
  }, [messages]);

  const showWaitingResponse =
    loading && hasUserMessage && !hasStreamingAssistantOutput;
  ...
  return (
    <MessageContext.Provider value={memoizedContext}>
      <Messages
        ...
        actions={actions}
      />
      {showWaitingResponse ? <WaitingResponse /> : null}
