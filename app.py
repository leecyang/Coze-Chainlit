...
# 本地开发：data/chainlit.db 或 .chainlit/chainlit.db
if os.path.exists("/app/data"):
    # Docker 环境
    DB_PATH = "/app/data/chainlit.db"
elif os.path.exists("data/chainlit.db"):
    # 本地开发，data 目录已有数据库
    DB_PATH = "data/chainlit.db"
elif os.path.exists("data"):
    # 本地开发，data 目录存在但数据库不存在
    DB_PATH = "data/chainlit.db"
else:
    # 回退到 .chainlit 目录
...
COZE_REDIRECT_URL = os.getenv("COZE_REDIRECT_URL", "http://localhost:8000/oauth/callback")
...
# Token priority: JWT > OAuth
# JWT: Long-term service account (recommended for production)
# OAuth: User-specific token with refresh

# User data storage
# Format: {username: {"password": str, "role": str, "coze_token": str, "coze_refresh_token": str, "token_expires_at": int}}
users_db: Dict[str, Dict[str, Any]] = {}

# OAuth state storage (temporary)
oauth_states: Dict[str, str] = {}  # state -> username
...
ADMIN_USERNAME = os.getenv("ADMIN_USERNAME", "yang")
ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "lcy@050426")

# Configuration storage for runtime updates
config_storage = {
    "COZE_BOT_ID": COZE_BOT_ID,
    "COZE_CLIENT_ID": COZE_CLIENT_ID,
    "COZE_CLIENT_SECRET": COZE_CLIENT_SECRET,
}
...
# 确保管理员用户始终存在（环境变量中的管理员优先级最高）
users_db[ADMIN_USERNAME] = {
    "password": ADMIN_PASSWORD,
    "role": "admin",
    "coze_token": users_db.get(ADMIN_USERNAME, {}).get("coze_token"),
    "coze_refresh_token": users_db.get(ADMIN_USERNAME, {}).get("coze_refresh_token"),
    "token_expires_at": users_db.get(ADMIN_USERNAME, {}).get("token_expires_at")
}
...
def log_activity(username: str, action: str):
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute(
            'INSERT INTO app_activity_logs (username, action) VALUES (?, ?)',
            (username, action)
        )
        conn.commit()
        conn.close()
...
def _load_config_from_db():
    ...
    global COZE_CLIENT_ID, COZE_CLIENT_SECRET, COZE_REDIRECT_URL
    ...
    try:
        ...
        if config_storage.get("COZE_CLIENT_ID"):
            COZE_CLIENT_ID = config_storage["COZE_CLIENT_ID"]
        if config_storage.get("COZE_CLIENT_SECRET"):
            COZE_CLIENT_SECRET = config_storage["COZE_CLIENT_SECRET"]
        if config_storage.get("COZE_REDIRECT_URL"):
            COZE_REDIRECT_URL = config_storage["COZE_REDIRECT_URL"]
...
async def refresh_coze_token(username: str) -> Optional[str]:
    """Refresh Coze OAuth token if expired"""
    user_data = users_db.get(username)
    if not user_data:
        return None
    
    # Check if using OAuth token (not API Key)
    current_token = user_data.get("coze_token")
    refresh_token = user_data.get("coze_refresh_token")
    expires_at = user_data.get("token_expires_at")
    
    # If no refresh token, it's using API Key mode
    ...
    print(f"[OAuth] Token expired or expiring soon for {username}, refreshing...")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(token_url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    # Update user data
                    users_db[username]["coze_token"] = new_access_token
                    users_db[username]["coze_refresh_token"] = new_refresh_token
                    users_db[username]["token_expires_at"] = int(datetime.now().timestamp()) + expires_in
                    
                    print(f"[OAuth] Token refreshed successfully for {username}, expires in {expires_in} seconds")
                    return new_access_token
                else:
                    error_text = await response.text()
                    print(f"[OAuth] Token refresh failed: {error_text}")
...
async def get_valid_token(username: str) -> Optional[str]:
    """Get valid token for user with multi-level fallback
    
    Priority for admin:
    1. User's OAuth token (with auto-refresh)
    2. System Service Identity token (long-term)
    
    Priority for regular user:
    1. System Service Identity token (long-term)
    """
    user_data = users_db.get(username)
    role = user_data.get("role", "user") if user_data else "user"
    
    if role == "admin":
        # Admin: Try OAuth first (user-specific)
        oauth_token = await refresh_coze_token(username)
        if oauth_token:
            return oauth_token
        print(f"[Token] Admin OAuth not available, falling back to system token")
    
    # All users (including admin fallback): Use system token
...
async def check_token_health() -> Dict[str, Any]:
    """Check token health status and return diagnostics
    
    Returns:
        Dict with token status and recommendations
    """
    health = {
        "service_identity": {"available": False, "valid": False, "expires_in": None},
        "oauth": {"available": False, "valid": False, "count": 0},
        "recommendation": ""
    }
    
    # Check Service Identity token
    if COZE_JWT_TOKEN:
        health["service_identity"]["available"] = True
        if COZE_JWT_EXPIRES_AT:
            try:
                expires_at = int(COZE_JWT_EXPIRES_AT)
                current_time = int(datetime.now().timestamp())
                expires_in = expires_at - current_time
                health["service_identity"]["expires_in"] = expires_in
                health["service_identity"]["valid"] = expires_in > 3600
            except (ValueError, TypeError):
                pass
        else:
            health["service_identity"]["valid"] = True
    
    # Check OAuth tokens
    oauth_count = 0
    for username, data in users_db.items():
        if data.get("coze_token") and data.get("coze_refresh_token"):
            oauth_count += 1
    health["oauth"]["available"] = oauth_count > 0
    health["oauth"]["count"] = oauth_count
    
    # Generate recommendation
    if health["service_identity"]["valid"]:
        health["recommendation"] = "✅ Service Identity token active - production ready"
    elif health["oauth"]["available"]:
        health["recommendation"] = "⚠️ Using OAuth tokens - ensure auto-refresh is working"
    else:
        health["recommendation"] = "❌ No valid token configured!"
...
async def oauth_callback(request: Request):
    """Handle OAuth callback from Coze"""
    ...
    # Verify state and get username
    username = oauth_states.get(state)
    if not username:
        return HTMLResponse(content=f"""
        <html>
        <head><title>授权失败</title></head>
    ...
    payload = {
        ...
        "client_id": COZE_CLIENT_ID,
        "client_secret": COZE_CLIENT_SECRET,
        "redirect_uri": COZE_REDIRECT_URL
    }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(token_url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    # Store token in user database with expiration time
                    if username in users_db:
                        users_db[username]["coze_token"] = access_token
                        users_db[username]["coze_refresh_token"] = refresh_token
                        users_db[username]["token_expires_at"] = int(datetime.now().timestamp()) + expires_in
                        print(f"[OAuth] Token stored for {username}, expires in {expires_in} seconds")
                        
                        # 记录 OAuth 授权活动
                        log_activity(username, "完成 OAuth 授权")
                    ...
                    <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
                        ...
                        <p>用户 <strong>{username}</strong> 已成功完成 Coze OAuth 授权。</p>
                        <p>您现在可以关闭此窗口并返回聊天界面使用 OAuth 功能了。</p>
                        <script>
                            setTimeout(function() {{
                                window.close();
                            }}, 3000);
...
async def on_message(message: cl.Message):
    ...
    if content == "/oauth":
        if role == "admin":
            ...
            await cl.Message(content="❌ 只有管理员可以使用 OAuth 功能。").send()
        return
    ...
    if content == "/config" and role == "admin":
        await configure_oauth()
        return
...
async def show_help(role: str):
    ...
    if role == "admin":
        ...
        help_text += "- `/config` - 配置 OAuth 参数 (Client ID, Client Secret)\n"
        help_text += "- `/oauth` - 开始 Coze OAuth 授权流程\n"
...
async def list_users():
    ...
    while True:
        ...
        for idx, (username, data) in enumerate(users_db.items(), 1):
            ...
            oauth_status = "✅ 已授权" if data.get("coze_token") else "❌ 未授权"
...
async def show_user_details():
    ...
    details += f"**OAuth 状态：** {oauth_status}\n"
    
    if data.get("token_expires_at"):
        import datetime
        expires_at = datetime.datetime.fromtimestamp(data["token_expires_at"])
        details += f"**Token 过期时间：** {expires_at.strftime('%Y-%m-%d %H:%M:%S')}\n"
    
    await cl.Message(content=details).send()


async def start_oauth_flow():
    """Start Coze OAuth authorization flow for admin"""
    import secrets
    ...
    oauth_url = (
        ...
        f"client_id={COZE_CLIENT_ID}&"
        f"redirect_uri={COZE_REDIRECT_URL}&"
        f"state={state}"
    )

    await cl.Message(
        content=f"🔗 **Coze OAuth 授权**\n\n"
                f"请点击以下链接完成授权：\n\n[{oauth_url}]({oauth_url})\n\n"
                f"授权完成后，页面会显示成功信息，您可以关闭页面返回聊天界面。"
    ).send()
...
async def configure_oauth():
    """Configure OAuth parameters (Client ID and Client Secret)"""
    global COZE_CLIENT_ID, COZE_CLIENT_SECRET

    await cl.Message(content="🔐 **配置 OAuth 参数**\n\n请输入 Coze Client ID：").send()
    ...
    if not res or not res.get("output"):
        await cl.Message(content="❌ 配置已取消。").send()
        return

    new_client_id = res.get("output").strip()

    await cl.Message(content="请输入 Coze Client Secret：").send()
    ...
    new_client_secret = res.get("output").strip()

    # Update configuration
    COZE_CLIENT_ID = new_client_id
    COZE_CLIENT_SECRET = new_client_secret
    config_storage["COZE_CLIENT_ID"] = new_client_id
    config_storage["COZE_CLIENT_SECRET"] = new_client_secret
    _save_config_to_db("COZE_CLIENT_ID", new_client_id)
    _save_config_to_db("COZE_CLIENT_SECRET", new_client_secret)

    # Auto-configure redirect URL based on server IP
    import socket
    try:
        # Get server IP
        hostname = socket.gethostname()
        server_ip = socket.gethostbyname(hostname)
        global COZE_REDIRECT_URL
        COZE_REDIRECT_URL = f"http://{server_ip}:8123/oauth/callback"
    except:
        # Fallback to default
        COZE_REDIRECT_URL = "http://localhost:8123/oauth/callback"

    await cl.Message(
        content=f"✅ **OAuth 参数配置成功！**\n\n"
                f"Client ID: {new_client_id[:10]}...\n"
                f"Client Secret: {'*' * 10}\n"
                f"Redirect URL: {COZE_REDIRECT_URL}\n\n"
                f"请在 Coze 开发者平台中将上述 Redirect URL 添加到 OAuth 回调地址列表中。"
    ).send()
...
async def admin_page(request: Request):
    ...
    if not admin_username:
        return HTMLResponse(
            ...
            <!DOCTYPE html>
            <html lang="zh-CN">
            <head><meta charset="UTF-8"><title>403 禁止访问</title></head>
            <body style="font-family: 'Inter', sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #0f0f23; color: #e2e8f0;">
                <div style="text-align: center;">
                    <h1 style="font-size: 72px; margin: 0; color: #9242eb;">403</h1>
                    <p style="font-size: 20px; color: #94a3b8;">禁止访问 - 您没有管理员权限</p>
                    <a href="/" style="color: #9242eb; text-decoration: none; font-size: 16px;">返回首页</a>
                </div>
            </body>
            </html>
            """,
    ...
    admin_html_path = Path(__file__).parent / "templates" / "admin.html"
...
async def get_admin_users(request: Request):
    ...
    for username, data in users_db.items():
        ...
        if data.get("token_expires_at"):
            try:
                expires_dt = datetime.fromtimestamp(data["token_expires_at"])
                token_expires_at = expires_dt.strftime("%Y-%m-%d %H:%M:%S")
            except:
                token_expires_at = "无效"
        ...
        users_list.append({
            ...
            "has_oauth": bool(data.get("coze_token")),
...
async def batch_create_users(request: Request):
    ...
    try:
        ...
        for row in reader:
            ...
            _save_user_to_db(username, password, role)
            log_activity(username, "被管理员批量导入创建")
...
async def update_admin_user(username: str, request: Request):
    ...
    try:
        ...
        if body.get("password"):
            ...
            log_activity(username, "被管理员修改了密码")
        ...
        if body.get("role"):
            ...
            log_activity(username, "被管理员修改了角色")
...
async def get_admin_config(request: Request):
    ...
    return JSONResponse({
        "bot_id": COZE_BOT_ID,
        "service_token": COZE_JWT_TOKEN,  # 前端期望 service_token
        "jwt_expires_at": jwt_expires_at,
        "base_url": COZE_BASE_URL,
        "client_id": COZE_CLIENT_ID,
        "client_secret": COZE_CLIENT_SECRET,
        "redirect_url": COZE_REDIRECT_URL
    })
...
async def update_admin_config(request: Request):
    ...
    try:
        ...
        if "client_id" in body:
            COZE_CLIENT_ID = body["client_id"]
            config_storage["COZE_CLIENT_ID"] = body["client_id"]
            _save_config_to_db("COZE_CLIENT_ID", body["client_id"])
        
        if "client_secret" in body:
            COZE_CLIENT_SECRET = body["client_secret"]
            config_storage["COZE_CLIENT_SECRET"] = body["client_secret"]
            _save_config_to_db("COZE_CLIENT_SECRET", body["client_secret"])
        
        if "redirect_url" in body:
            COZE_REDIRECT_URL = body["redirect_url"]
            config_storage["COZE_REDIRECT_URL"] = body["redirect_url"]
            _save_config_to_db("COZE_REDIRECT_URL", body["redirect_url"])
...
async def get_admin_conversations(request: Request):
    ...
    conversations = []
    # 预加载人设使用记录（thread_id → 最近使用的 persona）
    thread_personas = {}
    try:
        import sqlite3
        conn = sqlite3.connect(DB_PATH)
        ...
        for row in cursor:
            tid, persona = row
            thread_personas[tid] = persona  # 后面的记录覆盖前面的，保留最新
        conn.close()
    except Exception as e:
        print(f"[Admin Conversations] 加载人设映射失败: {e}")
    
    try:
        data_layer = get_data_layer()
        if data_layer:
            # 直接查询数据库获取所有线程（绕过 userId 限制）
            from sqlalchemy import text
            async with data_layer.async_session() as session:
                # 获取所有线程及其消息数（只计算用户消息和助手消息）
                query = text("""
                    SELECT 
                        t.id,
                        t.name,
                ...
                for row in rows:
                    thread_id, name, user_identifier, created_at, metadata, message_count = row
                    
                    # 解析 metadata 获取用户名与 Coze 对话 ID
                    ...
                    if metadata:
                        try:
                            ...
                            username = meta_dict.get('username') or user_identifier or "未知"
                            conversation_id = meta_dict.get('conversation_id')
                        except Exception as parse_e:
                            print(f"[Admin] 解析 metadata 失败: {parse_e}, 内容: {metadata}")
                    
                    # 获取该对话的人设 (persona_usage_logs 中的 thread_id 可能存了 Chainlit thread ID 或者 Coze conversation ID)
                    # 如果没有记录到人设，则显示默认人设
                    persona = ""
                    if conversation_id and conversation_id in thread_personas:
                        persona = thread_personas[conversation_id]
                    elif thread_id in thread_personas:
                        persona = thread_personas[thread_id]
                    if not persona:
                ...
                print(f"[Admin Conversations] Loaded {len(conversations)} conversations")
    except Exception as e:
    ...
    try:
        ...
        if data_layer:
            ...
            async with data_layer.async_session() as session:
                ...
                for row in rows:
                    step_id, name, step_type, output, created_at, metadata = row
                ...
                print(f"[Admin Conversation Detail] Loaded {len(messages)} messages for thread {thread_id}")
    except Exception as e:
