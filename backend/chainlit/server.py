...
def get_build_dir(local_target: str, packaged_target: str) -> str:
    ...
    env_key = "CHAINLIT_COPILOT_DIST" if packaged_target == "copilot" else "CHAINLIT_FRONTEND_DIST"
    explicit_build_dir = os.getenv(env_key)
    sibling_root = os.path.dirname(APP_ROOT)
    sibling_build_dir = (
        os.path.join(sibling_root, "frontend", "libs", "copilot", "dist")
        if packaged_target == "copilot"
        else os.path.join(sibling_root, "frontend", "dist")
    )
    ...
    elif explicit_build_dir and os.path.exists(explicit_build_dir):
        return explicit_build_dir
    elif os.path.exists(sibling_build_dir):
        return sibling_build_dir
...
async def get_logo(theme: Optional[Theme] = Query(Theme.light)):
    ...
    if not logo_path:
        logo_path = os.path.join(build_dir, f"logo_{theme_value}.svg")
