...
async def migrate_database(engine):
    ...
    async with engine.begin() as conn:
        ...
        try:
            result = await conn.execute(text("PRAGMA table_info(app_activity_logs)"))
            columns = [row[1] for row in result.fetchall()]
            for column in ("actor", "target", "detail"):
                if columns and column not in columns:
                    print(f"发现 app_activity_logs 表缺少 {column} 字段，正在添加...")
                    await conn.execute(text(f'ALTER TABLE app_activity_logs ADD COLUMN "{column}" TEXT'))
            print("app_activity_logs 表结构已是最新")
        except Exception as e:
            print(f"审计日志迁移检查中出错 (可能表尚未创建): {e}")
